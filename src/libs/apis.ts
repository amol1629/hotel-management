...
// Function to create a new room in Sanity
// Create Room Function
export const createRoom = async (roomData: Omit<Room, '_id' | 'slug'>) => {
  // Validate that the room has at least 3 images
  if (!roomData.images || roomData.images.length < 3) {
    throw new Error('At least 3 images are required for the room');
  }

  // Prepare the mutation to create a new room in Sanity
  const mutation = {
    mutations: [
      {
        create: {
